...
function searchTable(type) {
    // تحديد خانة البحث بناءً على النوع
    let inputId = "";
    if (type === 'animals') inputId = 'searchAnimal';
    else if (type === 'workers') inputId = 'searchWorker';
    else if (type === 'feeds') inputId = 'searchFeed';
    else if (type === 'visits') inputId = 'searchVisit';
...
function addAnimal() {
    ...
    if (!type || !age || !health || !code) return alert("أكمل البيانات");
    if (code.length !== 10) return alert("الكود 10 أرقام");
    let data = Storage.get("animals");
    const obj = { id: editType === "animal" ? editId : Date.now(), type, age, health, code };
    if (editType === "animal") data = data.map(a => a.id === editId ? obj : a);
    else data.push(obj);
    Storage.set("animals", data);
    renderAnimals(); clearInputs();
}
function renderAnimals() {
    let data = Storage.get("animals");
    let table = `<tr><th>النوع</th><th>العمر</th><th>الصحة</th><th>الكود</th><th>إجراءات</th></tr>`;
    data.forEach(a => { table += `<tr><td>${a.type}</td><td>${a.age}</td><td>${a.health}</td><td>${a.code}</td><td><button onclick="editAnimal(${a.id})">✏️</button><button onclick="deleteAnimal(${a.id})">🗑</button></td></tr>`; });
...
function addWorker() {
    ...
    if (!name || !job || !phone) return alert("أكمل البيانات");
    ...
    renderWorkers(); clearInputs();
}
function renderWorkers() {
    let data = Storage.get("workers");
    let table = `<tr><th>الاسم</th><th>الوظيفة</th><th>الهاتف</th><th>إجراءات</th></tr>`;
    data.forEach(w => { table += `<tr><td>${w.name}</td><td>${w.job}</td><td>${w.phone}</td><td><button onclick="editWorker(${w.id})">✏️</button><button onclick="deleteWorker(${w.id})">🗑</button></td></tr>`; });
...
function addVisit() {
    ...
    if (!vet || !animal || !date || !diag) return alert("أكمل البيانات");
...
function renderVisits() {
    let data = Storage.get("visits");
    let table = `<tr><th>الطبيب</th><th>رقم الحيوان</th><th>التاريخ</th><th>التشخيص</th><th>إجراءات</th></tr>`;
